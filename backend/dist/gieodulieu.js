...
async function gieoNguoiDung() {
    ...
    const admin = await prisma_js_1.prisma.nguoiDung.create({ data: { email: emailAdmin, matKhau, hoTen: 'Quan tri Effort Job', soDienThoai: '0900000001', vaiTro: 'admin', trangThai: 'hoat_dong' } });
    const ungVienUser = await prisma_js_1.prisma.nguoiDung.create({ data: { email: emailUngVien, matKhau, hoTen: 'Nguyen Minh Ung Vien', soDienThoai: '0900000002', vaiTro: 'ung_vien', trangThai: 'hoat_dong' } });
    const nhaTuyenDungUser = await prisma_js_1.prisma.nguoiDung.create({ data: { email: emailNhaTuyenDung, matKhau, hoTen: 'Tran Nha Tuyen Dung', soDienThoai: '0900000003', vaiTro: 'nha_tuyen_dung', trangThai: 'hoat_dong' } });
    return { admin, ungVienUser, nhaTuyenDungUser };
}
function skill(kyNang, ten, batBuoc = true) {
...
async function main() {
    ...
    const ungVien = await prisma_js_1.prisma.ungVien.create({
        data: {
    ...
    const congTy = await prisma_js_1.prisma.nhaTuyenDung.create({
        data: {
    ...
    const jobs = await Promise.all([
        prisma_js_1.prisma.tinTuyenDung.create({
            data: {
    ...
    const cv = await prisma_js_1.prisma.hoSoNangLuc.create({
        data: {
    ...
    const ungTuyen = await prisma_js_1.prisma.hoSoUngTuyen.create({
    ...
    await prisma_js_1.prisma.lichSuHoSoUngTuyen.createMany({
        data: [
    ...
    const lich = await prisma_js_1.prisma.lichPhongVan.create({
    ...
    await prisma_js_1.prisma.danhGiaCongTy.create({
    ...
    await prisma_js_1.prisma.viecLamDaLuu.create({ data: { maNguoiDung: ungVienUser.id, maTinTuyenDung: jobs[1].id, ngayLuu: new Date() } });
    ...
    const cuocTroChuyen = await prisma_js_1.prisma.cuocTroChuyen.create({
        data: {
