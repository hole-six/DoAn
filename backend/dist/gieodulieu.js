...
async function taoTaiKhoanMau() {
    ...
    const admin = await prisma_js_1.prisma.nguoiDung.create({ data: { email: emailAdmin, matKhau, hoTen: 'Quản trị Effort Job', soDienThoai: '0900000000', vaiTro: 'admin', trangThai: 'hoat_dong' } });
    const ungVienUsers = await Promise.all(danhSachUngVien.map((item, index) => prisma_js_1.prisma.nguoiDung.create({
        data: {
            email: item.email,
            matKhau,
            hoTen: item.hoTen,
...
async function main() {
    ...
    await Promise.all(candidateProfiles.map((ungVien, index) => taoKyNangUngVien(ungVien.id, [
    ...
    for (const [index, company] of companies.entries()) {
        for (const template of jobTemplates) {
            jobs.push(await prisma_js_1.prisma.tinTuyenDung.create({
                data: {
    ...
    const interviews = await Promise.all(applications.slice(0, 5).map((hoSo, index) => prisma_js_1.prisma.lichPhongVan.create({
        data: {
    ...
    await prisma_js_1.prisma.danhGiaCongTy.createMany({
    ...
    const chat = await prisma_js_1.prisma.cuocTroChuyen.create({
        data: {
