...
import { Portfolio } from './portfolio.mohinh.js'

type ThemePortfolio = 'professional' | 'modern' | 'creative'

type DuLieuPortfolio = {
...
const fontChoPhep: Record<string, string> = {
...
function escapeHtml(giaTri: unknown) {
...
function markdownThanhHtml(markdown: string) {
  const dong = String(markdown ?? '').replace(/\r\n/g, '\n').split('\n')
  const html: string[] = []
  let dangList = false

  const dongList = (noiDung: string) => {
    if (!dangList) {
      html.push('<ul>')
      dangList = true
    }
...
function chuanHoaPortfolio(taiLieu: any) {
...
async function layUngVienTuToken(authorization?: string) {
  const nguoiDung = await layNguoiDungTuAccessToken(authorization)
  if (nguoiDung.vaiTro !== 'ung_vien') throw new LoiUngDung('Chi ung vien moi duoc tao portfolio', 403)

  const ungVien = await (UngVien as any)
...
async function layHoSoThuocUngVien(maHoSoNangLuc: string, maUngVien: unknown) {
  const hoSo = await (HoSoNangLuc as any).findOne({ _id: maHoSoNangLuc, maUngVien })
  if (!hoSo) throw new LoiUngDung('Ho so nang luc khong ton tai hoac khong thuoc ve ban', 404)
  return hoSo
}
...
function mangThongTin(items: any[] = []) {
...
function renderMuc(tieuDe: string, items: Array<Record<string, string>>, rong: string) {
...
function renderPortfolioHtml(portfolio: any, ungVien: any, hoSo: any) {
  ...
  const theme: ThemePortfolio = portfolio.theme ?? 'professional'
  const kyNang = (ungVien.kyNang ?? []).map((muc: any) => muc.maKyNang?.tenKyNang).filter(Boolean)
  const duAnUngVien = (ungVien.portfolio ?? []).map((item: any) => ({
    tieuDe: item.tenDuAn,
    donVi: item.lienKet,
  ...
  <main class="page">
    ...
    <div class="grid">
      <aside class="stack">
        <section class="panel">
          <h2>Skills</h2>
          <div class="skills">${kyNang.length ? kyNang.map((kn: string) => `<span class="skill">${escapeHtml(kn)}</span>`).join('') : '<p class="muted">Chua cap nhat ky nang.</p>'}</div>
        </section>
...
export const dichVuPortfolio = {
  async layDanhSach(authorization?: string) {
    const ungVien = await layUngVienTuToken(authorization)
    const danhSach = await (Portfolio as any).find({ maUngVien: ungVien._id }).sort({ ngayCapNhat: -1 }).limit(50)
    return danhSach.map(chuanHoaPortfolio)
  },

  async layTheoMa(authorization: string | undefined, ma: string) {
    const ungVien = await layUngVienTuToken(authorization)
    const portfolio = await layPortfolioThuocUngVien(ma, ungVien._id)
    return chuanHoaPortfolio(portfolio)
  },

  async taoMoi(authorization: string | undefined, duLieu: DuLieuPortfolio) {
    const ungVien = await layUngVienTuToken(authorization)
    const hoSo = await layHoSoThuocUngVien(String(duLieu.maHoSoNangLuc), ungVien._id)
    const ketQua = await (Portfolio as any).create({
  ...
  async capNhat(authorization: string | undefined, ma: string, duLieu: DuLieuPortfolio) {
    const ungVien = await layUngVienTuToken(authorization)
    const portfolio = await layPortfolioThuocUngVien(ma, ungVien._id)
    const patch: Record<string, unknown> = { ...duLieu }
    if (duLieu.maHoSoNangLuc) {
      const hoSo = await layHoSoThuocUngVien(duLieu.maHoSoNangLuc, ungVien._id)
      patch.maHoSoNangLuc = hoSo._id
    }
    if (duLieu.mauChinh) patch.mauChinh = safeColor(duLieu.mauChinh, portfolio.mauChinh)
    if (duLieu.mauPhu) patch.mauPhu = safeColor(duLieu.mauPhu, portfolio.mauPhu)
    if (duLieu.font) patch.font = fontChoPhep[duLieu.font] ? duLieu.font : portfolio.font
    const ketQua = await (Portfolio as any).findByIdAndUpdate(ma, patch, { returnDocument: 'after', runValidators: true })
    return chuanHoaPortfolio(ketQua)
  },

  async preview(authorization: string | undefined, ma: string, duLieu: DuLieuPortfolio) {
    const ungVien = await layUngVienTuToken(authorization)
    const portfolio = await layPortfolioThuocUngVien(ma, ungVien._id)
  ...
  async exportHtml(authorization: string | undefined, ma: string) {
    const ungVien = await layUngVienTuToken(authorization)
    const portfolio = await layPortfolioThuocUngVien(ma, ungVien._id)
