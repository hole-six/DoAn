...
function layKyNang(kyNang: { id: string; tenKyNang: string }[], ten: string, batBuoc = true) {
  const item = kyNang.find(muc => muc.tenKyNang === ten)
  if (!item) return null
  return { maKyNang: item.id, batBuoc, mucDo: batBuoc ? 4 : 3 }
}

async function taoKyNangUngVien(maUngVien: string, danhSach: Array<{ maKyNang: string; mucDo?: number } | null>) {
...
async function taoKyNangTin(maTinTuyenDung: string, danhSach: Array<{ maKyNang: string; batBuoc?: boolean; mucDo?: number } | null>) {
  const rows = danhSach.filter(Boolean).map(item => ({
    maTinTuyenDung,
    maKyNang: item!.maKyNang,
...
async function main() {
    ...
    danhSachNhaTuyenDung.map((item, index) =>
      prisma.nhaTuyenDung.create({
        data: {
          ...
          lyDoTuChoi: (trangThaiCongTy[index] as any).lyDoTuChoi,
        },
      }),
    ),
  )
  ...
  const getK = (ten: string) => kyNangMap.get(ten) ?? null
  ...
  for (let ci = 0; ci < companies.length; ci++) {
    ...
    for (let ji = 0; ji < numJobs; ji++) {
      ...
      const job = await prisma.tinTuyenDung.create({
        data: {
  ...
  for (let i = 0; i < Math.min(10, candidateProfiles.length); i++) {
    ...
    await prisma.lichSuHoSoUngTuyen.create({
  ...
  const interviews = await Promise.all(
    moiPhongVanApps.map((hoSo, index) =>
  ...
  for (let i = 0; i < Math.min(5, companies.length); i++) {
    ...
    if (!app) continue
    const dg = await prisma.danhGiaCongTy.create({
      data: {
        maUngVien: candidateProfiles[i].id,
  ...
  if (chatApp) {
    const cuocChat = await prisma.cuocTroChuyen.create({
      data: {
