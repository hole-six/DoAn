...
export function Drawer({ title, onClose, children, footer }: { title: string; onClose: () => void; children: ReactNode; footer?: ReactNode }) {
  return (
    <div className="uv-shared-drawer fixed inset-0 z-[300] bg-slate-950/45 backdrop-blur-sm" onClick={onClose}>
      <aside className="uv-shared-drawer-card ml-auto flex h-dvh w-full max-w-[720px] flex-col bg-white shadow-2xl" onClick={event => event.stopPropagation()}>
        <header className="uv-shared-drawer-header flex min-h-16 items-center justify-between border-b border-slate-200 px-4">
        ...
        <div className="uv-shared-drawer-body min-h-0 flex-1 overflow-y-auto p-4">{children}</div>
        {footer && <footer className="uv-shared-drawer-footer border-t border-slate-200 bg-slate-50 p-4">{footer}</footer>}
      </aside>
    </div>
  )
}
