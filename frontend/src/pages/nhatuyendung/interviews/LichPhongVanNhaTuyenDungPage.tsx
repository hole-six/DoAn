...
export default function LichPhongVanNhaTuyenDungPage() {
  ...
  const openChat = async (item: LichPhongVan) => {
    const userId = (item.hoSoUngTuyen?.ungVien?.nguoiDung as any)?.id
      ?? (item.hoSoUngTuyen?.ungVien?.nguoiDung as any)?._id
      ?? (item.hoSoUngTuyen?.ungVien?.nguoiDung as any)
    if (!userId) return
          ...
          {data.interviews.length ? data.interviews.map(item => (
            <article key={item.id} className="grid gap-3 rounded-xl border border-slate-200 bg-white p-4 lg:grid-cols-[minmax(0,1fr)_auto]">
              <div className="min-w-0">
                ...
                <p className="mt-1 truncate text-sm font-semibold text-slate-500">{item.hoSoUngTuyen?.tinTuyenDung?.tieuDe ?? 'Phỏng vấn'} · {item.hoSoUngTuyen?.tinTuyenDung?.nhaTuyenDung?.tenCongTy ?? '-'}</p>
                <p className="mt-1 text-sm font-semibold text-slate-500">{formatDateTime(item.thoiGianBatDau)} · {item.linkHop || item.diaChi || '-'}</p>
              </div>
              <ButtonGroup>
                <Button size="sm" variant="secondary" icon={<MessageCircle size={15} />} disabled={['hoan_thanh', 'da_huy'].includes(item.trangThai)} onClick={() => void openChat(item)}>Nhắn ứng viên</Button>
