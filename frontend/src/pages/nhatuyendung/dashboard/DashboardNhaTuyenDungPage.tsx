    ...
    <Page
      ...
      action={(
        <>
          ...
          {gate.allowed ? (
            <Button variant="primary" icon={<Plus size={16} />} onClick={() => navigate('/nha-tuyen-dung/quan-ly-tin?new=1')}>Đăng tin</Button>
          ) : (
            <Button variant={gate.status === 'pending' ? 'secondary' : 'primary'} icon={<Building2 size={16} />} disabled={gate.status === 'pending'} onClick={() => navigate('/nha-tuyen-dung/cong-ty')}>
              {gate.cta}
            </Button>
          )}
        </>
      ...
      {!gate.allowed && (
        <Panel>
          <div className="grid gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4 text-amber-900 sm:grid-cols-[auto_minmax(0,1fr)_auto] sm:items-center">
            ...
            <Button variant="primary" icon={<Building2 size={16} />} disabled={gate.status === 'pending'} onClick={() => navigate('/nha-tuyen-dung/cong-ty')}>
              {gate.cta}
            </Button>
          </div>
        </Panel>
        ...
        <Panel title="Hồ sơ cần xử lý">
          <div className="grid gap-2">
            {pending.length ? pending.slice(0, 6).map(item => (
              <button key={item.id} className="grid gap-2 rounded-xl border border-slate-200 bg-white p-3 text-left hover:bg-slate-50 sm:grid-cols-[minmax(0,1fr)_auto]" onClick={() => navigate('/nha-tuyen-dung/ung-vien')}>
        ...
        <Panel title="Lịch phỏng vấn">
          <div className="grid gap-2">
            {upcoming.length ? upcoming.slice(0, 6).map(item => (
              <button key={item.id} className="grid gap-2 rounded-xl border border-slate-200 bg-white p-3 text-left hover:bg-slate-50 sm:grid-cols-[minmax(0,1fr)_auto]" onClick={() => navigate('/nha-tuyen-dung/lich-phong-van')}>
